...
func MakeAIStatsList(aiAdd, humanAdd, inTokens, outTokens int64, avgPrompt float64) string {
	...
	b.WriteString("```text\n")
	b.WriteString(fmt.Sprintf("🤖 Generated by AI:        %s lines\n", addCommas(int(aiAdd))))
	b.WriteString(fmt.Sprintf("👤 Written by Hand:        %s lines\n", addCommas(int(humanAdd))))
	b.WriteString(fmt.Sprintf("📊 AI Contribution:        %.1f%%\n", aiContribution))
	b.WriteString(fmt.Sprintf("🔤 Tokens In / Out:        %s / %s\n", humanizeCount(inTokens), humanizeCount(outTokens)))
	b.WriteString(fmt.Sprintf("💬 Avg Prompt:             %d chars\n", int(math.Round(avgPrompt))))
	b.WriteString("```\n\n")
