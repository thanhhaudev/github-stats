...
func MakeCodingStreakList(s *wakatime.AllTimeSinceTodayStats, currentStreak, longestStreak int) string {
	...
	fmt.Fprintf(&b, "🏆 Longest Streak:        %d days\n", longestStreak)
	...
	if s != nil {
		...
		fmt.Fprintf(&b, "📊 Daily Average:         %d hrs %d mins\n", dailyAvgHours, dailyAvgMinutes)
		fmt.Fprintf(&b, "💪 Total Coding Time:     %s\n", s.Data.Text)
		fmt.Fprintf(&b, "🎯 Coding Consistency:    %.1f%%\n", consistencyPercent)
		fmt.Fprintf(&b, "📅 Active Days:           %s days\n", addCommas(activeDays))
	}
